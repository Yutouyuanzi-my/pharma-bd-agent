    ...
    if trace:
        ...
        for step in trace:
            if step.get("tool") == "analyze_competitive_landscape":
                ...
                if top_sponsors:
                    df_sp = {"申办方": [s["sponsor"][:12] for s in top_sponsors],
                             "试验数": [s["trial_count"] for s in top_sponsors]}
                    st.subheader("🏢 主要申办方试验分布")
                    st.bar_chart(df_sp, x="申办方", y="试验数", use_container_width=True)

                if phase_dist:
                    p_counts = {p: len(v) for p, v in phase_dist.items() if v}
                    if p_counts:
                        st.subheader("📊 各阶段试验分布")
                        st.bar_chart({"阶段": list(p_counts.keys()), "试验数": list(p_counts.values())},
                                     x="阶段", y="试验数", use_container_width=True)

                # 从 landscape 数据中提取风险标记
                risk_items = []
                for sp in top_sponsors:
                    for t in sp.get("trials", []):
                        tags = t.get("risk_tags", [])
                        if tags:
                            risk_items.append({"NCT": t["nct_id"], "申办方": sp["sponsor"],
                                               "标签": ", ".join(tags)})
                if risk_items:
                    st.subheader("⚠️ 风险/价值事件")
                    st.table(risk_items)
                break

        # 渲染 search_clinical_trials 的表格
        for step in trace:
            if step.get("tool") == "search_clinical_trials":
                try:
                    raw = step.get("result_preview", "{}")
                ...
                if studies:
                    st.subheader("📋 检索结果")
                    table = "| NCT ID | 标题 | 申办方 | 阶段 | 状态 | 标签 |\n|---|---|---|---|---|---|\n"
                    for s in studies[:10]:
                        nct = s.get("nct_id", "")
                        link = s.get("nct_link", f"https://clinicaltrials.gov/study/{nct}")
                        title_t = s.get("brief_title", "")[:55]
                        sp = s.get("sponsor", "")[:18]
                        ph = s.get("phase", "") or "-"
                        sts = s.get("overall_status", "") or "-"
                        tags = ", ".join(s.get("risk_tags", [])) or "-"
                        table += f"| [{nct}]({link}) | {title_t} | {sp} | {ph} | {sts} | {tags} |\n"
                    st.markdown(table)
                    st.caption("⚠ = 已终止/安全性信号 | 🔬 招募中 | 🧬 First-in-Human | 📈 Phase 2 完成")
                break

        # 渲染 monitor_recent_changes 的表格
        for step in trace:
            if step.get("tool") == "monitor_recent_changes":
                try:
                    raw = step.get("result_preview", "{}")
                ...
                studies = data.get("studies", [])
                if studies:
                    since = data.get("since_date", "")
                    st.subheader(f"📅 近期更新（自 {since}）")
                    table = "| NCT ID | 标题 | 申办方 | 阶段 | 状态 | 标签 |\n|---|---|---|---|---|---|\n"
                    for s in studies[:10]:
                        nct = s.get("nct_id", "")
                        link = s.get("nct_link", f"https://clinicaltrials.gov/study/{nct}")
            ...
            if step.get("tool") == "search_pubmed":
                ...
                if articles:
                    ...
                    for a in articles[:5]:
                        pmid = a.get("pmid", "")
                        link = a.get("pubmed_link", f"https://pubmed.ncbi.nlm.nih.gov/{pmid}/")
                        title_t = a.get("title", "")[:50]
                        abstract = a.get("abstract", "")[:80].replace("\n", " ")
                        table += f"| [{pmid}]({link}) | {title_t} | {abstract} |\n"
                    st.markdown(table)
                break
